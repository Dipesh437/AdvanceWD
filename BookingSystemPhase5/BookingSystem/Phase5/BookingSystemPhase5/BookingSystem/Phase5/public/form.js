  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        // IMPROVED: friendly validation message
        showFormMessage("error", `❌ Please fix the following errors:\n\n${msg}\n\nCheck each field and try again.`);
        return;
      }

      // 409 = duplicate resourceName
      if (response.status === 409) {
        const details = body?.details || "A resource with the same name already exists.";
        // IMPROVED: friendly duplicate message, no status code
        showFormMessage("info", `⏳ Duplicate name\n\n${details}\n\nPlease choose a different name.`);
        return;
      }
    ...
    // -----------------------------------------
    // IMPROVED: friendly success message
