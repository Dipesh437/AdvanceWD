...
const app = express();
const PORT = process.env.PORT || 3000;  // FIXED: Added fallback
...
app.post("/api/resources", (req, res) => {
  ...
  console.log("Description ➡️ ", description);  // Now shows actual description
  console.log("Price ➡️ ", price);
  console.log("Price unit ➡️ ", unit);
  console.log("--------------------------");
  
  // Return structured response that matches what form.js expects
  return res.json({ 
    ok: true, 
    echo: {
...
app.use("/api", (req, res) => {
  res.status(404).json({ error: "Not found" });  // FIXED: Removed duplicate response
});
