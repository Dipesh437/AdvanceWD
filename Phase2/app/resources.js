...
// ===============================
// 4) Resource Description Validation (ADDED)
// ===============================
...
function attachResourceDescriptionValidation(textarea) {
  const update = () => {
    ...
    if (raw.trim() === "") {
      setInputVisualState(textarea, "invalid");
      updateCreateButtonState();
      return;
    }
...
function updateCreateButtonState() {
  ...
  const descTextarea = document.getElementById("resourceDescription");
  
  if (!nameInput || !descTextarea || !createButton) return;
  
  const nameValid = isResourceNameValid(nameInput.value);
  const descValid = isResourceDescriptionValid(descTextarea.value);
...
// Initialize description validation (ADDED)
const descriptionTextarea = document.getElementById("resourceDescription");
if (descriptionTextarea) {
  attachResourceDescriptionValidation(descriptionTextarea);
}
